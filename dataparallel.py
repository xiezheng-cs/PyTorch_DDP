...
def main_worker(args):
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        epoch_end = time.time()
        logger.info('|==> Epoch={{:d}/{:d}}, best_acc1={}, best_acc1_index={}, time_cost={:.4f}'
...
def train(train_loader, model, criterion, optimizer, epoch, args, logger, writer):
    ...
    for i, (images, target) in enumerate(train_loader):
        ...
        if i % args.print_freq == 0:
            logger.info('Train epoch: [{:d}/{:d}][{:d}/{:d}]\tlr={:.6f}\tce_loss={:.4f}\ttop1_acc={:.4f}'
                        .format(epoch, args.epochs, i, len(train_loader), get_learning_rate(optimizer), losses.avg, top1.avg))
    ...
    logger.info('||==> Train epoch: [{:d}/{:d}]\tlr={:.6f}\tce_loss={:.4f}\ttop1_acc={:.4f}'
...
def validate(val_loader, model, criterion, args, logger, writer, epoch):
    ...
    with torch.no_grad():
        ...
        for i, (images, target) in enumerate(val_loader):
            ...
            if i % args.print_freq == 0:
                logger.info('Val epoch: [{:d}/{:d}][{:d}/{:d}]\tce_loss={:.4f}\ttop1_acc={:.4f}'
                            .format(epoch, args.epochs, i, len(val_loader), losses.avg, top1.avg))

        # save tensorboard
        writer.add_scalar('Val_ce_loss', losses.avg, epoch)
        writer.add_scalar('Val_top1_accuracy', top1.avg, epoch)

        logger.info('||==> Val epoch: [{:d}/{:d}]\tce_loss={:.4f}\ttop1_acc={:.4f}'
...
parser.add_argument('-j', '--workers', default=4, type=int, metavar='N', help='number of data loading workers (default: 4)')
parser.add_argument('--epochs', default=90, type=int, metavar='N', help='number of total epochs to run')
parser.add_argument('--start-epochs', default=0, type=int, metavar='N', help='manual epoch number ()')
...
def train(train_loader, model, criterion, optimizer, epoch, args, logger, writer):
    ...
    for i, (images, target) in enumerate(train_loader):
        ...
        if i % args.print_freq == 0:
            logger.info('Train epoch: [{:d}/{:d}][{:d}/{:d}]\tlr={:.6f}\tce_loss={:.4f}\ttop1_Acc={:.4f}\ttop5_Acc={:.4f}'
                    .format(epoch, args.epochs, i, len(train_loader), get_learning_rate(optimizer), losses.avg, top1.avg, top5.avg))

    # save tensorboard
    writer.add_scalar('lr', get_learning_rate(optimizer), epoch)
    writer.add_scalar('Train_ce_loss', losses.avg, epoch)
    writer.add_scalar('Train_top1_accuracy', top1.avg, epoch)
    writer.add_scalar('Train_top5_accuracy', top5.avg, epoch)

    logger.info('||==> Train epoch: [{:d}/{:d}]\tlr={:.6f}\tce_loss={:.4f}\ttop1_Accuracy={:.4f}'
                    .format(epoch, args.epochs, get_learning_rate(optimizer), losses.avg, top1.avg, top5.avg))
...
def validate(val_loader, model, criterion, args, logger, writer, epoch):
    ...
    with torch.no_grad():
        ...
        for i, (images, target) in enumerate(val_loader):
            ...
            if i % args.print_freq == 0:
                logger.info(
                    'Val epoch: [{:d}/{:d}][{:d}/{:d}]\tce_loss={:.4f}\ttop1_Acc={:.4f}\ttop5_Acc={:.4f}'
                    .format(epoch, args.epochs, i, len(val_loader), losses.avg, top1.avg, top5.avg))

        # save tensorboard
        writer.add_scalar('Val_ce_loss', losses.avg, epoch)
        writer.add_scalar('Val_top1_accuracy', top1.avg, epoch)
        writer.add_scalar('Val_top5_accuracy', top5.avg, epoch)

        logger.info('||==> Train Epoch: [{:d}/{:d}]\tce_loss={:.4f}\ttop1_Accuracy={:.4f}'
                    .format(epoch, args.epochs, losses.avg, top1.avg, top5.avg))
        return top1.avg
